...
const coldObservable = new Observable<string>((observer) => {
	...
	pages.forEach((page) => {
		observer.next(page); // Por cada pagina se envia informacion
	});
...
export const coldSubscriptions = () => {
	...
	coldObservable.subscribe((page) => console.log(`Leyendo Lector 1: ${page}`));

	// Después de 2 segundos, se suscribe un segundo lector
	setTimeout(() => {
		coldObservable.subscribe((page) => console.log(`Leyendo Lector 2: ${page}`));
	}, 2000);
};